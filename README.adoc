...
The Restaurant application is designed with reactive architecture. The application that you'll be working with consists of five microservices : `Order`, `Kitchen`, `Bar`, `ServingWindow`, and `OpenLibertyCafe`. It also uses Kafka broker to enable reactive messaging communication between the Producer and Consumer microservices over the messaging channels.
...
docker build -t restaurantbff:1.0-SNAPSHOT OpenLibertyCafe/.
```
