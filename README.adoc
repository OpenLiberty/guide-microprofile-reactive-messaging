...
The [hotspot=kafkaConfig]`mp.messaging.connector.liberty-kafka.bootstrap.servers` property defines the Kafka connector for the Open Liberty runtime. The `Order` microservice uses three Outgoing streams: [hotspot=foodStream]`food`, [hotspot=bevStream]`beverage`, and [hotspot=updateStatus]`updateStatus`. All these streams connect to the connector [hotspot=kafka1 hotspot=kafka2 hotspot=kafka3]`liberty-kafka`. The [hotspot=topic1 hotspot=topic2 hotspot=topic3]`topic` property defines the topics in Kafka, which are used to communicate with the `Kitchen`, `Bar`, and `Status`  microservices. The [hotspot=serializer1 hotspot=serializer2 hotspot=serializer3]`key.serializer` and [hotspot=serializerVal1 hotspot=serializerVal2 hotspot=serializerVal3]`value.serializer` properties characterize how to serialize the messages. Similarly `key.deserializer` and `value.deserializer` properties define how to deserialize the messages. The `group.id` defines a unique name for the consumer group. All these properties are required by the https://kafka.apache.org/documentation/#api[Apache Kafka client API^].
...
The `Kitchen`, and `Status` microservices are put together in this section.
