...
== Building Producer in the System microservice
...
== Building Consumer in the Inventory microservice
...
----
The `Inventory` microservice listens to messages from an incoming stream [hotspot=systemLoad file=1]`systemLoad`, wherein the messages are sent by the `System` microservice through the [hotspot=topic1 file=0]`systemLoadTopic` topic in the Kafka messaging-broker. Similarly the [hotspot=deserializer1 file=1]`key.deserializer` and [hotspot=deserializerVal1 file=1]`value.deserializer` properties define how to deserialize the messages.
...
You can rerun the http://localhost:9085/inventory/systems[^] URL after fifteen seconds, and you will notice the CPU usage properties for all the systems have changed.
