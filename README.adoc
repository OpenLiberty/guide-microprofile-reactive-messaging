...
`kitchen`, `bar` and `servingWindow` microservices are built here.
...
If the order is a food, `kitchen` service receives an Order on [hotspot=initFoodOrder file=0]`@Incoming("foodOrderConsume")` channel, processes it in [hotspot=initFoodOrder file=0]`initFoodOrder()` method and sends back `IN_PROGRESS` status on [hotspot=initFoodOrder file=0]`@Outgoing("foodOrderPublishIntermediate")` and `READY` status on [hotspot=foodOrder file=0]`@Outgoing("foodOrderPublish")` to Kafka over MicroProfile Reactive Messaging channels.
...
If the order is a drink, `bar` service receives an Order on [hotspot=initBevOrder file=1]`@Incoming("bevOrderConsume")` channel, processes it in [hotspot=initBevOrder file=1]`initBeverageOrder()` method and sends back `IN_PROGRESS` status on [hotspot=initBevOrder file=1]`@Outgoing("bevOrderPublishInter")` and `READY` status on [hotspot=bevOrder file=1]`@Outgoing("beverageOrderPublish")` to Kafka over MicroProfile Reactive Messaging channels.
