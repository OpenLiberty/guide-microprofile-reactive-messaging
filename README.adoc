...
You'll update order , kitchen , bar and servingWindow microservices to use MP-microprofile reactive messaging for message passing. These microservices run on Open Liberty.
...
The waiter places a request using a `restaurantBFF` service.

`order` service consumes the request, produces an order message and sends it to Kafka on food or beverage channel depending on the type. 

The Order begins with a NEW status. `kitchen` and `bar` microservices consume and processes the order and updates the status to IN_PROGRESS and READY consecutively. There’s a sleep time in between each status and the updates are reported back to the order Service via reactive messages on Kafka.

`servingWindow` Microservice contains a list of all ready to serve food and beverages. It consumes these statuses from kitchen and bar services.
Once the order is served, it’s marked as COMPLETED and the status is sent back to the order service as a message.
