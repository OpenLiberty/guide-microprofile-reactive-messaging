...
You will build and run the `System`, `Job`, `OpenLibertyCafe`, and `Inventory` microservices in Docker containers.
...
Make a `POST` request to the `/api/jobs` endpoint. To make this request, expand the `POST` endpoint on the UI and click the *Try it out button*. Copy the following example input into the text box, and then click the `Execute` button. The `POST` request adds a new `food` Job.
