...
You’ll update methods in `order`,`bar`, `ServingWindowResource` and `kitchen` services to use `@Outgoing` and `@Incoming` MP-ReactiveMessaging annotations.
...
----
#microprofile-config.properties sample from  the kitchen microservice.#
...
Rest all the microservices are configure similarly
...
Rest all the microservices have already configured `server.xml`
...
. MP-microprofile dependency is included in the Maven [hotspot=reactiveMessaging file=1]`pom.xml` file. This is specified in the [hotspot=reactiveMessaging file=0]`<dependency>` element.
...
Click `Execute` and you will see the response with `COMPLETED` status 
...
As you can see the order `0002` has been marked `COMPLETED` in `servingWindow` microservice and the update is cascaded to the `order` microservice 
