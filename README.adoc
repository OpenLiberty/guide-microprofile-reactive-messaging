...
Here we define [hotspot=kafkaConfig]`liberty-kafka.bootstrap` to use local kafka. [hotspot=foodStream file=0]`@Outgoing food` stream sends food as a String to the `foodTopic`. Similarly [hotspot=bevStream file=0]`@Outgoing beverage` is sent to the `beverageTopic`. [hotspot=updateStatus file=0]`@Incoming updateStatus` reads the statuses from `statusTopic`.
