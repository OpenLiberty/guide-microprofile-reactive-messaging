...
The `order` service uses a microprofile reactive messaging to send messagew to `bar` and `kitchen` microservices over kafka.
You will update method in `order`,`bar` and `kitchen` service to use MP-ReactiveMessaging to use `@Outgoing` and `@Incoming` annotations.

`order` creates an Order and sends it to the kafka topic on `beverage` or `food` channel. Depending on the type `bar` or `kitchen` receives an Order, processes it and sends back all the statuses back to kafka on MP-reactive messaging channels.
