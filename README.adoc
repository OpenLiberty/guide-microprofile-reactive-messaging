...
You'll update the `Order`, `Kitchen`, `Bar`, `ServingWindow`, and `Status` microservices to use MicroProfile Reactive Messaging for message passing. These microservices run on Open Liberty.
...
The `Order` microservice consumes the request, produces order messages, and sends them to the Kafka on the `food` or `beverage` channel depending on the type. 
...
The `ServingWindow` microservice illustrates when the food and beverages are ready to be served. It consumes the `READY` order from the `Kitchen` and `Bar` microservices. Once the order is served, waiter can mark the order as `COMPLETED` and the status is sent back to the `Status` microservice as a message.
...
The `Status` microservice receives the message from the `Order`, `Bar`, `Kitchen`, and `ServingWindows` microservices over the [hotspot=updateStatus file=3]`@Incoming("updateStatus")` channel with its properties defined in the [hotspot=updateStatus file=4]`microprofile-config.properties`.
