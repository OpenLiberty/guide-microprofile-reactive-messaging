...
Click `Execute` and you receive the 200 response.

Now Expand the `/api/orders GET request to get the Order status` and click *Try it out*.

Click `Execute` and you shall see the response with `IN_PROGRESS` status 
...
Click `Execute` again and you shall see the response with `READY` status 
