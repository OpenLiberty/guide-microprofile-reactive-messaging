...
Each of the five microservices `Bar`, `Kitchen`, `Order`, `ServingWindow`, and `Status` contain `microprofile-config.properties`
...
The [hotspot=kafkaConfig file=0]`mp.messaging.connector.liberty-kafka.bootstrap.servers` property defines the Kafka connector for the Open Liberty runtime. The `Bar` microservice uses an outgoing stream [hotspot=beverageOrderPublish file=0]`beverageOrderPublishStatus`, which sends messages to the [hotspot=topic1 file=0]`statusTopic` topic in the Kafka messaging-broker, so that the `ServingWindow` and `Status` microservices can consume the messages. The `Bar` microservice listens messages from an incoming stream [hotspot=bevOrderConsume file=0]`beverageOrderConsume`, which messages are sent by the `Order` microservice through the [hotspot=topic2 file=0]`beverageTopic` topic in the Kafka messaging-broker. The [hotspot=serializer1 file=0]`key.serializer` and [hotspot=serializerVal1 hotspot=serializerVal2 file=0]`value.serializer` property characterizes how to serialize the messages. Similarly [hotspot=deserializerVal2 file=0]`value.deserializer` property defines how to deserialize the messages. The [hotspot=group file=0]`group.id` defines a unique name for the consumer group. All these properties are required by the https://kafka.apache.org/documentation/#api[Apache Kafka client API^].
