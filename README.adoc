...
Both the `system` and `inventory` microservices contain the `microprofile-config.properties` file. MicroProfile Reactive Messaging uses connectors to connect certain channels to Apache Kafka and setting properties on those connectors.
...
----
The `inventory` microservice listens to messages from an incoming stream [hotspot=systemLoad file=1]`systemLoad`, wherein the messages are sent by the `system` microservice through the [hotspot=topic1 file=1]`systemLoadTopic` topic in the Kafka messaging-broker. Similarly the [hotspot=deserializer1 file=1]`key.deserializer` and [hotspot=deserializerVal1 file=1]`value.deserializer` properties define how to deserialize the messages. The custom [hotspot=deserializerVal1 file=1]`value.deserializer` is defined in the [hotspot=jsonbDeSerializer file=2]`SystemLoadDeserializer()` method of the `SystemLoad` class.
...
You can rerun the http://localhost:9085/inventory/systems[^] URL after a while, and you will notice the CPU usage properties for all the systems have changed.
