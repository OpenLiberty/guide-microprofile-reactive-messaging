...
Here we define [hotspot=kafkaConfig]`liberty-kafka.bootstrap` to use local kafka. [hotspot=foodStream]`@Outgoing food` stream sends food as a String to the `foodTopic`. Similarly [hotspot=bevStream]`@Outgoing beverage` is sent to the `beverageTopic`. [hotspot=updateStatus]`@Incoming updateStatus` reads the statuses from `statusTopic`.
...
- In order to develop the MicroProfile Reactive Messaging application using maven, the [hotspot=reactiveMessaging file=1]`microprofile-reactive-messaging-api` and [hotspot=kafka file=1]`kafka-clients` dependencies needs to be added to the pom.xml
