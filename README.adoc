...
The application also requires Apache Kafka however you will be using Kafka Docker containers to simplify the installation and configuration process.
...
[hotspot=reactiveMessaging file=2]`microprofile-reactive-messaging-api` dependency is needed to enable the use of MicroProfile Reactive Messaging API. [hotspot=kafka file=2]`kafka-clients` dependency is added since the application needs a Kafka client to connect to the Kafka broker. [hotspot=rxjava file=2]`rxjava` dependency is used for creating events at regular intervals.
