...
`connector` is a special property in Reactive Messaging, which defines [hotspot=kafkaConfig]`liberty-kafka.bootstrap`. `bootstrap.servers`, `group.id`, `key.deserializer`, and `value.deserializer` are properties understood by the Kafka client API.
