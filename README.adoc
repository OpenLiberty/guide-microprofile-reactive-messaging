...
To access the `inventory` microservice, use the http://localhost:9085/inventory/systems[^] URL, and you see the CPU systemLoad properties for all the systems.
...
You can rerun the http://localhost:9085/inventory/systems[^] URL after a while, and you will notice the CPU systemLoad properties for all the systems have changed.

You can use the `@GET` `\http://localhost:9085/inventory/system/{hostId}` URL to see the CPU usage properties for one particular system.
