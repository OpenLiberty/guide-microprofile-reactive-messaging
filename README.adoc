...
The OpenLiberty Cafe application that you'll be working with consists of four microservices : `Kitchen`, `Order`, `OpenLibertyCafe`, and `Status`. It also uses Kafka to enable reactive messaging communication between the Producer and Consumer microservices over the messaging channels.
...
The `Order` microservice uses the MicroProfile Reactive Messaging to send messages to `Kitchen` over Kafka.
