...
[hotspot=reactiveMessaging file=2]`microprofile-reactive-messaging-api` dependency is needed because MicroProfile umbrella spec doesn't include reactive messaging. [hotspot=kafka file=2]`kafka-clients` dependency is required since the application needs Kafka client to connect to the Kafka broker. [hotspot=rxjava file=2]`rxjava` dependency is necessary to create events at regular intervals and Open Liberty does not provide an simple way to build a publisher and regularly submit events.
