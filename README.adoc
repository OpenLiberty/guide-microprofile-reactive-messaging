...
The job manager application that you'll be working with consists of three important microservices : `System`, `Job`, and `Inventory`. It also uses Kafka to enable reactive messaging communication between the Producer and Consumer microservices over the messaging channels.

You'll update the `System`, `Job` and `Inventory` microservices to use MicroProfile Reactive Messaging for message passing. These microservices run on Open Liberty.

image::reactive-messaging-job.png[Reactive system inventory,align="center"]
...
The `Job` microservice allows you to dispatch jobs that will be run by the `System` microservice. It also consumes the request, produces job messages, and sends them to Kafka on the `job` channel.

A job begins with a `NEW` status. The `System` microservice illustrate how to handle the job as a real system. It consumes, processes the job and update the status to `IN_PROGRESS` and `COMPLETED` consecutively. There’s a sleep operation in between each status to represent the job processing time, and the status updates are reported back to the `Inventory` microservice via reactive messages on the Kafka.

The `Inventory` microservice receives status updates and provides queries for the user. All the jobs metadata are persisted here. Because persisting data is not the focus of this guide, the `Inventory` microservice simply uses a Map object to store the data.
...
`@Outgoing` is an annotation indicating that the method feeds a channel. The name of the channel is given as attribute. For example, `@Outgoing("job")`

*@Incoming*

`@Incoming` is an annotation indicating that the method consumes a channel. The name of the channel is given as attribute. For example, `@Incoming("jobConsume")`
...
image::reactive-messaging-job-detail.png[Reactive system inventory detail,align="center"]
...
Each of the three microservices `System`, `Job`, and `Inventory` contain `microprofile-config.properties`
...
=== Building the Job microservice

The `Job` microservice uses the MicroProfile Reactive Messaging to send messages to the `System` microservice over Kafka.

[role="code_command hotspot file=0", subs="quotes"]
----
#Replace the `JobResource` class.#
`job/src/main/java/io/openliberty/guides/job/JobResource.java`
----

JobResource.java
[source, Java, linenums, role='code_column hide_tags=copyright']
----
include::finish/job/src/main/java/io/openliberty/guides/job/JobResource.java[]
----

job/microprofile-config.properties
[source, Text, linenums, role='code_column hide_tags=copyright']
----
include::finish/job/src/main/resources/META-INF/microprofile-config.properties[]
----

The `Job` microservice creates jobs when a job is sent in a [hotspot=postJob file=0]`POST` request to the `/jobs` endpoint. The [hotspot=createJob file=0]`createJob()` method receives a `job` and adds it to the [hotspot=jobQueue file=0]`FlowableEmitter`. We use [hotspot=takeF file=0]`Flowable.create()` from rxJava to create a Publisher and an associated emitter. Objects we send to the emitter will be sent to the `Publisher`. We return the `Publisher` from our [hotspot=OutgoingJob file=0]`@Outgoing("job")` channel, which is configured in the [hotspot file=1]`microprofile-config.properties` [hotspot=jobStream file=1]`job` stream and MicroProfile Reactive Messaging takes care of assigning the publisher to the channel.
...
=== Completing the System and Inventory microservices

You will complete the `System` and `Inventory` microservices implementation in this section.

*Completing the System microservice*
...
The `System` service receives a job on the [hotspot=jobConsume file=0]`@Incoming("jobConsume")` channel, processes it in [hotspot=initJob file=0]`receiveJob()` method and sends back an `IN_PROGRESS` status on the [hotspot=jobPublishIntermediate file=0]`@Outgoing("jobPublishStatus")` channel to Kafka over MicroProfile Reactive Messaging channels. After a while, a `COMPLETED` status will be sent to the [hotspot=jobPublishStatus file=0]`@Outgoing("jobPublishStatus")` channel.

*Completing the Inventory microservice*
...
The `Inventory` microservice receives the message from the `System`, and `Job` microservices over the [hotspot=updateStatus file=1]`@Incoming("updateStatus")` channel with its properties defined in the [hotspot=updateStatus file=2]`microprofile-config.properties`.
...
You will build and run the `System`, `Job`, `Inventory`, and `Gateway` microservices in Docker containers.
