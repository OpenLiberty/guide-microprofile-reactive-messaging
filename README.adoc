...
The `Job` microservice consumes the request, produces job messages, and sends them to Kafka on the `job` channel.

A job begins with a `NEW` status. The `System` microservices illustrate how to handle the jobs as a real system. It consume and process the job and update the status to `IN_PROGRESS` and `COMPLETED` consecutively. There’s a sleep operation in between each status to represent the job processing time, and the status updates are reported back to the `Inventory` microservice via reactive messages on the Kafka.

The `Inventory` microservice receives status updates and provides queries for the user. All the jobs are persisted here. Because persisting data is not the focus of this guide, the `Inventory` microservice simply uses a Map object to store the data.
