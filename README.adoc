...
Using these properties [hotspot=foodStream]`@Outgoing food` stream sends food as a String to the `foodTopic`. Similarly, [hotspot=bevStream]`@Outgoing beverage` sends it to the `beverageTopic`. [hotspot=updateStatus]`@Incoming updateStatus` reads the statuses from `statusTopic`.
