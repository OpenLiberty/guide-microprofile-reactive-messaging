...
The OpenLiberty Cafe application is designed with reactive architecture. The application that you'll be working with consists of five microservices : `Order`, `Kitchen`, `Bar`, `ServingWindow`, and `OpenLibertyCafe`. It also uses Kafka to enable reactive messaging communication between the Producer and Consumer microservices over the messaging channels.
...
Once the Order is served, it is marked as `COMPLETED` and the status is sent back to the `Order` microservice as a message.
...
You will build and run the `Order`, `Kitchen`, `Bar`, `ServingWindow` and `OpenLibertyCafe` microservices in Docker containers.
...
Click `Execute` again and you will see the response with a `READY` status. Just in case if you see a `IN_PROGRESS` status then wait for a couple of seconds and click the `Execute` button again.
