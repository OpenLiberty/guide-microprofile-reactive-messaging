...
The `Inventory` microservice receives the message from the `System` microservices over the [hotspot=cpuStatus file=0]`@Incoming("cpuStatus")` channel with its properties defined in the [hotspot=cpuStatus file=1]`microprofile-config.properties`.
