...
An order begins with a `NEW` status. The `Kitchen` and `Bar` microservices illustrate how to handle orders as a real cafe. They consume and process the order and update the status to `IN_PROGRESS` and `READY` consecutively. There’s a sleep operation in between each status to represent the order processing time, and the status updates are reported back to the `Order` microservice via reactive messages on Kafka.

The `ServingWindow` microservice illustrates where the food and beverages are ready to serve. It consumes the `READY` order from the `Kitchen` and `Bar` microservices. Once the Order is served, waiter can mark the order to `COMPLETED` status and the status is sent back to the `Status` microservice as a message.

The `Status` microservice receives any status update and provides queries for waiter. It is the place to persist all orders data. Because persisting data is not the focus of this guide, the `Status` microservice simply uses a Map object to store the data.
...
`status/src/main/java/io/openliberty/guides/servingWindow/StatusResource.java`
----
...
The `Status` microservice receives message from the `Order`, `Bar`, `Kitchen`, `ServingWindows` microservices over the [hotspot=updateStatus file=3]`@Incoming("updateStatus")` channel with its properties defined in the [hotspot=updateStatus file=4]`microprofile-config.properties` receives updated order statuses from the Kafka topic.
