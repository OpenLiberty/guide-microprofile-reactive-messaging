...
----
#microprofile-config.properties example from the order microservice.#
`finish/order/src/main/resources/META-INF/microprofile-config.properties`
----

`connector` is a special property in Reactive Messaging, which defines [hotspot=kafkaConfig file=0]`liberty-kafka.bootstrap`. `bootstrap.servers`, `group.id`, `key.deserializer`, and `value.deserializer` are properties understood by the Kafka client API.
Using these properties [hotspot=foodStream file=0]`@Outgoing food` stream sends food as a String to the `foodTopic`. Similarly, [hotspot=bevStream file=0]`@Outgoing beverage` sends it to the `beverageTopic`. [hotspot=updateStatus file=0]`@Incoming updateStatus` reads the statuses from `statusTopic`.
...
- In order to develop the MicroProfile Reactive Messaging application using maven, the [hotspot=reactiveMessaging file=2]`microprofile-reactive-messaging-api` and [hotspot=kafka file=2]`kafka-clients` dependencies needs to be added to the pom.xml
